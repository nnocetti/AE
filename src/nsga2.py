...
class Evaluation:
    ...
    def evaluate(self, ind):
        ...
        target_coordinates = ['' for _ in range(len(self.sources))]
        # itero sobre los destinos
        for i in range(len(self.targets)):
            target_coordinates[ind[0][i]] += f";{self.targets[ind[1][i]][1]},{self.targets[ind[1][i]][2]}"
        # itero sobre los origenes
        urls = []
        for i in range(len(self.sources)):
            # Si hay algun envio desde ese origen armo la url
            if target_coordinates[i]:
                urls.append(f"http://{OSRM_HOST}/route/v1/driving/{self.sources[i][0]},{self.sources[i][1]}{target_coordinates[i]}")

        deliver_time = {}
        for u in urls:
            for i in range(3):
                try:
                    data = session.get(u).json()
            ...
            for i, leg in enumerate(data['routes'][0]['legs']):
                ...
                deliver_time[ind[1][i]] = self.targets[ind[1][i]][0] + duration
