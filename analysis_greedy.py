...
if __name__ == "__main__":
    ...
    (run_dirpath, _, run_filenames) = next(os.walk(f'{path}/runs'))
    (greedy1_dirpath, _, greedy1_filenames) = next(os.walk(f'{path}/1greedy'))
    (greedyx_dirpath, _, greedyx_filenames) = next(os.walk(f'{path}/xgreedy'))

    ref_front = ParetoFront()

    raw_data = {}
    for file in run_filenames:
        (instance, timestamp, runtime, cxmethod, cxpb, indpb, mu, ngen, seed, logbook, pop) = load_run_file(f'{run_dirpath}\{file}')
    ...
    for file in greedy1_filenames:
        (instance, timestamp, runtime, cxmethod, cxpb, indpb, mu, ngen, seed, logbook, pop) = load_run_file(f'{greedy1_dirpath}\{file}')
        ref_front.update(pop)
        param_id = 'ungreedy'
        raw_data[param_id] = raw_data[param_id] if param_id in raw_data else []
        run_front = ParetoFront()
    ...
    for file in greedyx_filenames:
        (instance, timestamp, runtime, cxmethod, cxpb, indpb, mu, ngen, seed, logbook, pop) = load_run_file(f'{greedyx_dirpath}\{file}')
        ref_front.update(pop)
        param_id = 'xgreedy'
        raw_data[param_id] = raw_data[param_id] if param_id in raw_data else []
        run_front = ParetoFront()
    ...
    params = ['random', 'ungreedy', 'xgreedy']
    rhv = {}
